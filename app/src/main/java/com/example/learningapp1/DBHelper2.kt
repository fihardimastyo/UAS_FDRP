...
class DBHelper2(context: Context): SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    companion object {
        ...
        private val SQL_CREATE_ENTRIES =
            "CREATE TABLE " + DBInfo2.UserInput.TABLE_NAME + " (" + DBInfo2.UserInput.COL_EMAIL +
                    " VARCHAR(200) PRIMARY KEY, " + DBInfo2.UserInput.COL_PASS + " TEXT, " +
                    DBInfo2.UserInput.COL_USERNAME + " VARCHAR(200), " + DBInfo2.UserInput.COL_FULLNAME +
                    " TEXT)"
    ...
    fun insertData(emailin: String, passin: String, usernamein: String, fullnamein: String): Boolean {
        ...
        val emailt = DBInfo2.UserInput.COL_EMAIL
        val passt = DBInfo2.UserInput.COL_PASS
        val usernamet = DBInfo2.UserInput.COL_USERNAME
        val fullnamet = DBInfo2.UserInput.COL_FULLNAME
...
//        val users = ArrayList<DBModel>()
        ...
        if (cursor!!.moveToFirst()){
            while (cursor.isAfterLast==false){
                emailt = cursor.getString(cursor.getColumnIndex(DBInfo2.UserInput.COL_EMAIL))
                passt = cursor.getString(cursor.getColumnIndex(DBInfo2.UserInput.COL_PASS))
                usernamet = cursor.getString(cursor.getColumnIndex(DBInfo2.UserInput.COL_USERNAME))
                fullnamet = cursor.getString(cursor.getColumnIndex(DBInfo2.UserInput.COL_FULLNAME))
